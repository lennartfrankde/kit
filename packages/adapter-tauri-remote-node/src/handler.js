...
/* global ENV_PREFIX, SERVE_STATIC */
...
/**
 * CORS middleware for Tauri apps
...
const cors = (req, res, next) => {
	...
	let allowed = false;
	if (corsConfig.origin && requestOrigin) {
		if (typeof corsConfig.origin === 'string') {
			allowed = requestOrigin === corsConfig.origin;
		} else if (Array.isArray(corsConfig.origin)) {
			allowed = corsConfig.origin.includes(requestOrigin);
		} else if (corsConfig.origin instanceof RegExp) {
			allowed = corsConfig.origin.test(requestOrigin);
		}
	}
...
// Build handler chain - always include CORS, conditionally include static serving
const handlers = /** @type {(import('sirv').RequestHandler | import('polka').Middleware)[]} */ ([
	cors
]);

if (SERVE_STATIC) {
	const clientServe = serve(path.join(dir, 'client'), true);
	const prerenderedServe = serve_prerendered();
	if (clientServe) handlers.push(clientServe);
	if (prerenderedServe) handlers.push(prerenderedServe);
}

handlers.push(ssr);

export const handler = sequence(handlers);
